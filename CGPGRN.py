...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('-me', '--multiple', dest = 'multipleExecutions', type = str,
                        default = 'None',
                        help='Perform execution of multiple problems on a given problem list \n')
...
if __name__ == '__main__':
    ...
    for splineDataName in splineDataNames:
        ...
        it_genesNamesFiles, it_directories, it_additional_discretizedFiles, it_discretizedFiles = CGPGRN.performDiscretizationStep(dirName, bestNClusters, currentPseudoTime, splineDataName, expressionDataFile, pseudotimeFile)
           ...
           if clusterMethod not in correlationMethods and clusterMethod != 'None':
               ...
               if discretizationApproach == 'BiKMeans':
                   ...
                   if fullTT == True:
                       ...
                       Log.register('message', "Using one output per gene...")
                       GO.generateOutputs(problemName, genesNamesFiles[i], discretizedFiles[i], 0, truthTableDirectory, 1, 1)
    ...
    if runCGP:
        ...
        if os.name != 'nt':
            ...
            currentExec = subprocess.call(shellFile, shell=True)
            currentExec.wait()
        else:
            currentExec = os.popen(allShellScriptsFileName).read()
